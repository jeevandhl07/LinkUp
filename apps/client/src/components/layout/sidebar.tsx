...
export const Sidebar = ({
  ...
  const runSearch = async (value: string) => {
    ...
    setDirectResults(
      users.filter((user) => !existingDirectPartnerIds.has(user.id)),
    );
  };
    ...
    const senderName =
      lastMessage.senderId === currentUser?.id
        ...
        : conversation.members.find(
            (member) => member.userId === lastMessage.senderId,
          )?.user?.name || "User";
  ...
  const section = (label: string, items: Conversation[]) => (
    <div>
      ...
      <div className="space-y-1">
        {items.length === 0 ? (
          <div className="rounded-md p-2 text-xs text-muted">
            No conversations
          </div>
        ) : null}
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="truncate text-xs text-muted">
                    {subtitleForConversation(conversation)}
                  </div>
                  {unreadCount > 0 ? (
            ...
            <Dialog.Portal>
              ...
              <Dialog.Content className="fixed left-1/2 top-1/2 w-[95vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border border-border bg-panel p-4">
                ...
                <form className="space-y-3" onSubmit={submitGroup}>
                  ...
                  <Input
                    value={groupQuery}
                    onChange={(e) => void runGroupSearch(e.target.value)}
                  ...
                  <div className="max-h-44 space-y-1 overflow-auto rounded-md border border-border p-2">
                    ...
                    {groupQuery.length >= 2 && groupResults.length === 0 ? (
        ...
        <div className="mb-3 rounded-lg border border-border p-2">
          ...
          <div className="space-y-1">
            {directResults.map((user) => (
              <button
                key={user.id}
                className="w-full rounded-lg p-2 text-left text-sm hover:bg-border/40"
                onClick={() => onCreateDirect(user.id)}
              >
            ...
            {directResults.length === 0 ? (
              <div className="p-2 text-xs text-muted">No people found</div>
            ) : null}
          </div>
        </div>
      ) : null}
