...
const shapeConversation = (
  ...
  if (conversation.type === "direct") {
    const other =
      members.find((member: any) => member.userId !== currentUserId) ||
      members[0];
...
const getLastMessage = async (conversationId: string) => {
  return MessageModel.findOne({ conversationId: toObjectId(conversationId) })
    .sort({ _id: -1 })
    .lean();
};
...
const getUnreadCount = async (conversation: any, userId: string) => {
  const member = (conversation.members || []).find(
    (item: any) => getMemberUserId(item) === userId,
  );
  if (!member) return 0;
...
const ensureAdmin = (conversation: any, userId: string) => {
  const membership = conversation.members.find(
    (member: any) => getMemberUserId(member) === userId,
  );
...
export const conversationService = {
  async createOrGetDirect(currentUserId: string, targetUserId: string) {
    ...
    let conversation = await ConversationModel.findOne({
      type: "direct",
      "members.userId": {
        $all: [toObjectId(currentUserId), toObjectId(targetUserId)],
      },
  ...
  async createGroup(
    currentUserId: string,
    input: { title: string; avatarUrl?: string; memberIds: string[] },
  ) {
  ...
  async addMembers(
    ...
    const existingIds = new Set(
      conversation.members.map((member: any) => getMemberUserId(member)),
    );
  ...
  async removeMember(
    ...
    if (conversation.type !== "group") {
      throw new AppError(
        "Only groups can remove members",
        StatusCodes.BAD_REQUEST,
      );
    }
    ...
    conversation.members = conversation.members.filter(
      (member: any) => getMemberUserId(member) !== targetUserId,
    ) as any;
    await conversation.save();
    await conversation.populate("members.userId", "_id name email avatarUrl");
    return shapeConversation(conversation, userId);
  ...
  async leaveConversation(userId: string, conversationId: string) {
    ...
    return {
      deleted: false,
      conversation: shapeConversation(conversation, userId),
    };
  },
